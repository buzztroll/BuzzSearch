...
class OAuthClient():
  ...
  def make_async_request(self, url, token="", secret="", additional_params=None,
    ...
    token = '://'
    ndx = url.find(token)
    quoted = urllib2.quote(url[ndx + len(token):])
    url2 = url[:ndx + len(token)] + quoted
  ...
  def get_user_info(self, auth_token, auth_verifier=""):
    ...
    if not auth_secret:
      ...
      if not result:
        raise Exception, "The auth token %s was not found in our db" % (auth_token)
      else:
  ...
  def _extract_credentials(self, result):
    ...
    if 'oauth_session_handle' in parsed_results:
